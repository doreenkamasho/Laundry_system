...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}"
    data-bs-theme="{{ session('user_' . auth()->id() . '_theme', 'light') }}"
    data-layout-style="{{ session('user_' . auth()->id() . '_layoutStyle', 'default') }}"
    data-sidebar="{{ session('user_' . auth()->id() . '_sidebar', 'light') }}"
    data-sidebar-size="{{ session('user_' . auth()->id() . '_sidebarSize', 'lg') }}"
    data-sidebar-image="{{ session('user_' . auth()->id() . '_sidebarImage', 'none') }}">

<head>
    <meta charset="utf-8" />
    <title>@yield('title') | LaundryHub Management System</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta content="Premium Multipurpose Admin & Dashboard Template" name="description" />
    <meta content="Themesbrand" name="author" />
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <meta name="user-avatar" content="{{ auth()->user()->avatar ? asset('storage/'.auth()->user()->avatar) : '' }}">
    <!-- App favicon -->
        ...
        @auth
            @switch(auth()->user()->role->name)
                ...
                @case('customer')
                    ...
                    @break
                @case('laundress')
                    @include('laundress.sidebar.sidebar')
                    @break
                @default
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const settings = {
        theme: '{{ session("user_" . auth()->id() . "_theme", "light") }}' || localStorage.getItem('theme'),
        layoutStyle: '{{ session("user_" . auth()->id() . "_layoutStyle", "default") }}' || localStorage.getItem('layout-style'),
        sidebar: '{{ session("user_" . auth()->id() . "_sidebar", "light") }}' || localStorage.getItem('sidebar'),
        sidebarSize: '{{ session("user_" . auth()->id() . "_sidebarSize", "lg") }}' || localStorage.getItem('sidebar-size'),
        sidebarImage: '{{ session("user_" . auth()->id() . "_sidebarImage", "none") }}' || localStorage.getItem('sidebar-image')
    };
    ...
    @yield('script')
    <script src="{{ URL::asset('build/libs/apexcharts/apexcharts.min.js') }}"></script>ild/libs/apexcharts/apexcharts.min.js') }}"></script>
    <!-- ... other scripts ... -->
    <script src="{{ URL::asset('build/js/pages/menu.init.js') }}"></script>    <script src="{{ URL::asset('build/js/pages/menu.init.js') }}"></script>
