...
@section('title') Reviews for {{ $laundress->name }} @endsection
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Reviews for {{ $laundress->name }}</h4>
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{ route('customer.dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('customer.find-laundress') }}">Find Laundress</a></li>
    ...
    <div class="row">
        <div class="col-xl-12">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <!-- Add Review Form -->
                    <form id="reviewForm" action="{{ route('customer.laundress.reviews.store', $laundress->id) }}" method="POST">
                        @csrf
                    ...
                    <div class="flex-shrink-0">
                        <div class="d-flex gap-2 align-items-center">
                            <div class="text-warning fs-16">
                                ...
                                <span id="averageRating">{{ number_format($laundress->average_rating, 1) }}</span>
                            </div>
                            <span class="text-muted">(<span id="reviewsCount">{{ $laundress->reviews_count }}</span> reviews)</span>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div id="reviewsList">
                        @if($reviews->isEmpty())
                            <div class="text-center py-4" id="noReviewsMessage">
                                ...
                                <p class="text-muted">Be the first to review {{ $laundress->name }}'s services!</p>
                            </div>
                        @else
                                ...
                                <div class="d-flex mb-4 pb-4 {{ !$loop->last ? 'border-bottom' : '' }} review-item" id="review-{{ $review->id }}">
                                    ...
                                    <div class="flex-grow-1 ms-3">
                                        ...
                                        <div class="comment-reactions mt-2">
                                            <div class="comment-reaction like-btn {{ $review->is_liked ? 'text-primary' : '' }}" data-review-id="{{ $review->id }}">
                                                <i class="ri-thumb-up-{{ $review->is_liked ? 'fill' : 'line' }}"></i>
                                                <span class="reaction-count">{{ $review->likes_count }}</span>
                                            </div>
                                            <div class="comment-reaction reply-btn" data-review-id="{{ $review->id }}">
                                                <i class="ri-reply-line"></i>
                                                <span>Reply</span>
                                                <span class="ms-1 reply-count">({{ $review->replies_count }})</span>
                                            </div>
                                        </div>
