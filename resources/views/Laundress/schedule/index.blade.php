    ...
    <div class="row">
        <div class="col-lg-4">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('laundress.schedule.availability') }}" method="POST">
                        @csrf
                        <div class="form-check form-switch form-switch-lg mb-3" dir="ltr">
                            <input type="checkbox" class="form-check-input" id="is_available" name="is_available" 
                                {{ $user->schedule->is_available ? 'checked' : '' }} onchange="this.form.submit()">
                            <label class="form-check-label" for="is_available">
                                I am {{ $user->schedule->is_available ? 'available' : 'not available' }} for bookings
                            </label>
                        </div>
                        <p class="text-muted">
                            ...
                            @foreach($upcomingBookings as $booking)
                                <a href="#" class="list-group-item list-group-item-action">
                                    <div class="d-flex w-100 justify-content-between">
                                        ...
                                        <small class="badge bg-{{ $booking->status_color }}">{{ ucfirst($booking->status) }}</small>
                                    </div>
                                    <p class="mb-1">{{ $booking->service_name }}</p>
                                    <small>{{ \Carbon\Carbon::parse($booking->scheduled_date)->format('M d, Y') }} at {{ \Carbon\Carbon::parse($booking->scheduled_time)->format('h:i A') }}</small>
                                </a>
                            @endforeach
                        </div>
                    @else
        ...
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('laundress.schedule.update') }}" method="POST">
                        @csrf
                        <div class="row">
                                    ...
                                    <div class="day-schedule">
                                        <div class="day-header">
                                            ...
                                            <div class="form-check form-switch">
                                                <input type="hidden" name="working_days[{{ $day }}][is_available]" value="0">
                                                <input class="form-check-input day-toggle" type="checkbox" 
                                                    id="{{ $day }}_available" 
                                                    name="working_days[{{ $day }}][is_available]" 
                                                    value="1"
                                                    data-day="{{ $day }}"
                                                    {{ $user->schedule->working_days[$day]['is_available'] ? 'checked' : '' }}>
                                                <label class="form-check-label" for="{{ $day }}_available">Available</label>
                                            </div>
                                        </div>
                                        <div class="time-slots {{ $user->schedule->working_days[$day]['is_available'] ? '' : 'd-none' }}" id="{{ $day }}_time_slots">
                                            <div class="row">
                                                <div class="col-6">
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="time" class="form-control" 
                                                            ...
                                                            value="{{ $user->schedule->working_days[$day]['start_time'] }}">
                                                    </div>
                                                </div>
                                                <div class="col-6">
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="time" class="form-control" 
                                                            ...
                                                            value="{{ $user->schedule->working_days[$day]['end_time'] }}">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            weekends: true,
            selectable: false,
            businessHours: {!! json_encode(collect(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'])
            ...
            events: {!! json_encode($upcomingBookings->map(function($booking) {
                return [
                    'id' => $booking->id,
